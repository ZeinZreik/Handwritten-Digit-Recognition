...
def prob_img(probs):
    fig, ax = plt.subplots()
    rects = ax.bar(range(len(probs)), probs, color=(0.09,0.88,0.91,1))
...
def correct():
    ...
    img = img.reshape((1, 28, 28))
    img = torch.tensor(img, dtype=torch.float).unsqueeze(0)
    img = np.concatenate(img.tolist()).ravel()
    img = np.concatenate(([label],img))
    #print(img)
    with open('data/train.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        # write the data
        writer.writerow(img)
    response = {
...
@app.route("/retrainorig", methods=["GET", "POST"])
def retrainorig():
    data_url = str(request.get_data())
    os.system('python trainOroginal.py')
    response = {
    }
    return jsonify(response)
